...
do
	if [[ "$memfrees" == "" ]]
	then
		memtotals=`echo $LINE |awk -F',' '{printf "%.2f", $2/1000}'`
		memfrees=`echo $LINE |awk -F',' '{printf "%.2f", $6/1000}'`
		actives=`echo $LINE |awk -F',' '{printf "%.2f", $12/1000}'`
	else
		memtotals=$memtotals","`echo $LINE |awk -F',' '{printf "%.2f", $2/1000}'`
		memfrees=$memfrees","`echo $LINE |awk -F',' '{printf "%.2f", $6/1000}'`
		actives=$actives","`echo $LINE |awk -F',' '{printf "%.2f", $12/1000}'`
	fi
done < MEM
