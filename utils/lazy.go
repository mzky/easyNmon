...
func GenIndexPage(nr *NmonReport, fPath string) {
	...
	sfs, _ := fs.New()
	if err := InitFile(sfs, filepath.Join("chart", "data.tpl"), filepath.Join(fPath, "data.json")); err != nil {
		log.Printf("%+v", err)
		return
	}
	//tpl := template.Must(template.New("data.tpl").ParseFiles(filepath.Join("web", "chart", "data.tpl")))
	tpl := template.New(filepath.Join(fPath, "data.json"))
