...
func main() {
	...
	flag.String("杀掉所有监控任务","","get示例：http://192.168.x.x:8080/kill\r\n")
	flag.String("查看报告","","get示例：http://192.168.x.x:8080/report\r\n")
	flag.String("退出程序","","get示例：http://192.168.x.x:8080/stop\r\n")
	flag.Parse()
	lsCmd := exec.Command("echo", "begin")
	...
	r.StaticFS("/report",http.Dir("./report/"))
		...
		//lsCmd.SysProcAttr = &syscall.SysProcAttr{Setpgid: true}
		...
		Cmd := exec.Command("/bin/sh", "-c", "ps -ef|grep nmon|grep -v grep|awk {'print $2'}|xargs kill -9")
		err := Cmd.Start()  
        	//syscall.Kill(-lsCmd.Process.Pid, syscall.SIGKILL)
                if err!=nil{
                       	fmt.Println(err)
                }	
                c.JSON(200, gin.H{
